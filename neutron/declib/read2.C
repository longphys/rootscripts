...
void read2(){
  ...
  for (int i = 0; i < sizeof(distance_from_center)/sizeof(distance_from_center[0]); i++){
    std::cout << i << "\n";
    gr1->AddPoint(distance_from_center[i], proton_cross_talk[i]);
    std::cout << "distance = " << distance_from_center[i] << "; cross-talk = " << proton_cross_talk[i] << "\n";
    gr2->AddPoint(distance_from_center[i], neutron_mis_hits_neighbor[i]);
    std::cout << "distance = " << distance_from_center[i] << "; mis_hits_neighbor = " << neutron_mis_hits_neighbor[i] << "\n";
    gr3->AddPoint(distance_from_center[i], neutron_mis_hits_center[i]);
    std::cout << "distance = " << distance_from_center[i] << "; mis_hits_center = " << neutron_mis_hits_center[i] << "\n";
  }

  c_1->cd();
  gr1->Draw("ALP");
  gr1->GetXaxis()->SetRangeUser(-1.0, 22.0);
